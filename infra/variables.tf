variable "aws_region" {
  ...
  type        = string
  default     = "us-east-1" # Ou outra região de preferência
}
...
variable "instance_type" {
  ...
  type        = string
  default     = "t2.micro" # Tipo gratuito ou de baixo custo para início
}

variable "ami_id" {
  description = "ID da AMI (Amazon Machine Image) para as instâncias EC2 (Ubuntu LTS)"
  type        = string
  # Exemplo para us-east-1, Ubuntu 22.04 LTS. Pode precisar ser atualizado.
  # Recomenda-se buscar o ID da AMI mais recente para a região escolhida.
